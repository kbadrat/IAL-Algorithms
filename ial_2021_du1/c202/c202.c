...
void Stack_Init( Stack *stack ) {
	...
	//Zkontroluji, zda je zasobnik prazdny. V tomto priprade zavolam funkci Stack_Error.
	if (stack == NULL){
		Stack_Error(SERR_INIT);
		return;
	}
	//Kdyz neni, tak nastavim index prvku vrcholu zasobniku na -1(zacatek zasobniku).
	else 
		stack->topIndex = -1;
}
...
int Stack_IsEmpty( const Stack *stack) {
	...
	//Vrat mi 1, zda je zasobnik prazdny(-1).
...
int Stack_IsFull( const Stack *stack ) {
	...
	//Vrat mi 1, zda je zasobnik plny(STACK_SIZE - 1).
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	//Zkontroluji, zda je zasobnik prazdny.
	if (Stack_IsEmpty(stack)){
		Stack_Error(SERR_TOP);
		return;
	}
	//Na adresu vkladam hodnotu z vrcholu zasobniku.
	else
...
void Stack_Pop( Stack *stack ) {
	//Zkontroluji, zda neni prazdny. V tomto pripade odebere z vrcholu zasobniku jeden prvek. 
	if (!Stack_IsEmpty(stack))
			stack->topIndex--;
}
...
void Stack_Push( Stack *stack, char data ) {
	...
	//Zkontroluji, zda je zasobnik plny.
	if (Stack_IsFull(stack))
		Stack_Error(SERR_PUSH);
	//Pokud neni, tak zvetsim index vrcholu zasobniku na 1 a pridam na vrchol hodnotu z data.  
	else
	{
		stack->topIndex++;
